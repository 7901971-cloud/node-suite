...
Cloudflare 向已登记的公网 IPv4/IPv6 和节点 TCP 端口建立连接（例如 38444），连接建立后再写入一个极小的探测字节；连接和写入都成功时显示“Cloudflare 可连接”。失败重试最多 3 次，每次超时 4 秒，不保留旧的成功结果。

完整上报、地址/端口变化、手动探测立即检查；普通心跳距上次探测满 5 分钟再次检查。Cloudflare 是单一外部网络，尤其对中国家宽 IPv6，失败只能证明当前 CF 出口到该地址的路径未连通，不能证明实际客户端无法使用。因此 IPv6 探测失败只在完整状态和外部探测中提示，不再将设备标黄；已有的 IPv6 探测告警会在新版 Worker 收到下一次心跳时自动解除。IPv4 探测仍参与异常判断，但任一可用公网路径成功时设备整体保持绿色。

设备健康状态以设备心跳、Xray 进程、本机 TCP 监听、可发布公网地址、DDNS、内存、温度、存储和负载为准；这些本机故障仍会正常标黄或离线标红。探测字节不包含 UUID、REALITY 公钥和 Short ID，只确认 CF 到端口的 TCP 路径，不等于 VLESS/REALITY 认证或实际代理上网测试。实现依据：[Cloudflare TCP sockets](https://developers.cloudflare.com/workers/runtime-apis/tcp-sockets/)。
