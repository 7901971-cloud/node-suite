...
read_settings() {
  ...
  REALITY_SNI="$(read_default 'REALITY SNI' "${OLD_SNI:-$DEFAULT_SNI}")"
  valid_hostname "$REALITY_SNI" || die "SNI 域名格式不正确。"
  REALITY_DEST="$(read_default 'REALITY 目标' "${OLD_DEST:-${REALITY_SNI}:443}")"
  [[ "$REALITY_DEST" =~ ^[A-Za-z0-9.-]+:[0-9]{1,5}$ ]] || die "REALITY 目标格式应为 域名:端口。"
  local dest_port="${REALITY_DEST##*:}"
  valid_port "$dest_port" || die "REALITY 目标端口不正确。"
  ...
  if (( ENABLE_MONITOR == 1 )); then
    ...
    DEVICE_NAME="$(read_default 'Telegram 中显示的设备名称' "${OLD_DEVICE_NAME:-$DEFAULT_DEVICE_NAME}")"
