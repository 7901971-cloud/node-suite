...
prepare_router_reality() {
    ...
    for host in $candidates; do
        ...
        while [ "$probe" -le 3 ]; do
            ...
            if grep -q 'Verify return code: 0 (ok)' "$log" && grep -q 'ALPN protocol: h2' "$log"; then
                case "$start_ms:$end_ms" in
                    *[!0-9:]*|:*) ms=999999 ;;
                    *) if [ "$end_ms" -ge "$start_ms" ]; then ms=$((end_ms - start_ms)); else ms=999999; fi ;;
                esac
        ...
        if [ "$fail" -eq 0 ] && [ -n "$a" ] && [ -n "$b" ] && [ -n "$c" ]; then
            x1=$a; x2=$b; x3=$c
            if [ "$x1" -gt "$x2" ]; then tmp=$x1; x1=$x2; x2=$tmp; fi
            if [ "$x2" -gt "$x3" ]; then tmp=$x2; x2=$x3; x3=$tmp; fi
            if [ "$x1" -gt "$x2" ]; then tmp=$x1; x1=$x2; x2=$tmp; fi
            median=$x2
            printf '  ✓ %-22s %6s / %6s / %6s ms  中位数 %6s ms\n' "$host:443" "$a" "$b" "$c" "$median"
            if [ -z "$best_host" ] || [ "$median" -lt "$best_ms" ]; then
                best_host="$host"
                best_ms="$median"
            fi
        else
    ...
    [ -n "$best_host" ] || die '12 个内置 REALITY 目标均未通过 3 次严格检查；尚未写业务配置'
    REALITY_SNI="$best_host"
    REALITY_DEST="$best_host:443"
    say "自动选择：SNI=$REALITY_SNI；目标=$REALITY_DEST；3 次握手中位数约 ${best_ms} ms"
